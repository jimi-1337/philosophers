...
/*   Updated: 2021/01/11 19:36:21 by amoujane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		...
		while (i < g_data.amount)
		{
			if (check >= g_data.amount)
				return (1);
			if (ft_time_test(ft_time(), g_philos[i].last_meal) > g_data.time_to_die)
			{
				g_philos[i].died = 1;
				ft_display_message("is died\n", g_philos[i].index);
				return (1);
			}
			if (g_times_eat_check && (g_philos[i].times_eat >= g_data.must_eat_count) && (g_philos[i].check != -1))
			{
				g_philos[i].check = -1;
				check++;
